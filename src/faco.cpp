...
public:
    ...
    void init_trail_limits_smooth() {
        ...
        trail_limits_.min_ = 1.0 / pow(problem_.dimension_, 1.0 / 4);
    ...
    double deposit_pheromone(const Ant &sol) {
        ...
        for (auto node : sol.route_) {
            ...
            //cout<<deposit << ' '<<trail_limits_.min_ << ' ' << trail_limits_.max_<<endl;
    ...
    void deposit_pheromone_smooth(const Ant &sol) {
        ...
        auto &pheromone = get_pheromone();
        for (auto node : sol.route_) {
            //cout<<deposit_smooth_ << ' '<<trail_limits_.min_ << ' ' << trail_limits_.max_<<endl;
...
public:
    ...
    */
    void relocate_node(uint32_t target, uint32_t node) {
        ...
        if (target_pos < node_pos) {  // Case 1.
            ...
            //cout<<end - beg<<' '<<node_pos - target_pos<<'\n';
            ...
            //cout<<end - beg<<' '<<target_pos - node_pos<<'\n';
            std::rotate(beg, beg + 1, end);
    ...
    void quick_relocate(uint32_t target, uint32_t start_node, uint32_t end_node) {
        if (get_succ(target) == start_node) return;
        const auto start_pos = positions_[start_node];
        const auto end_pos = positions_[end_node];
        auto target_pos = positions_[target];
        auto node_pred = get_pred(start_node);  
        const auto node_pred_rv = get_pred(end_node);
        auto node_succ = get_succ(end_node);
        const auto node_succ_rv = get_succ(start_node);  
        auto target_succ = get_succ(target);

        bool is_reverse;
        if ((target_pos < start_pos && target_pos > end_pos) || ((target_pos > start_pos && target_pos < end_pos))) {
            if (start_pos > end_pos) {
                is_reverse = false;
                _quick_relocate(target_pos, 0, end_pos);
                target_pos = positions_[target];
                _quick_relocate(target_pos, start_pos, route_.size() - 1);
            } else {
                is_reverse = true;
                _quick_relocate(target_pos, route_.size() - 1, end_pos);
                target_pos = positions_[target];
                _quick_relocate(target_pos, start_pos, 0);
            }
        } else {
            is_reverse = (start_pos > end_pos);
            _quick_relocate(target_pos, start_pos, end_pos);
        }


        // We are removing these edges:
        if (is_reverse) {
            node_pred = node_pred_rv;
            node_succ = node_succ_rv;
            if (node_pred == target) {
                target_succ = node_succ;
            }
            cost_ -= cost_fn_(node_pred, end_node) + cost_fn_(start_node, node_succ);
        } else {
            cost_ -= cost_fn_(node_pred, start_node) + cost_fn_(end_node, node_succ);
        }
        cost_ += - cost_fn_(target, target_succ)
                 + cost_fn_(target, start_node)
                 + cost_fn_(end_node, target_succ)
                 + cost_fn_(node_pred, node_succ);
    }

    void _quick_relocate(uint32_t target_pos, uint32_t start_pos, uint32_t end_pos) {
        const auto len = route_.size();
        if (start_pos > end_pos) {
            std::swap(start_pos, end_pos);
            std::reverse(route_.begin() + start_pos, route_.begin() + end_pos + 1);
        }
        if (target_pos < start_pos) {  
            // Case 1.
            // 1 2 3 t 5 6 n1 n2 n3 7 8 =>
            // 1 2 3 t n1 n2 n3 5 6 7 8
            auto beg = route_.begin() + target_pos + 1;
            auto mid = route_.begin() + start_pos;
            auto end = route_.begin() + end_pos + 1;
            std::rotate(beg, mid, end);

            for (auto i = target_pos; i <= end_pos; ++i) {
                positions_[route_[i]] = i;
            }
        } else { 
            // Case 2.
            // 1 2 3 n1 n2 n3 5 6 t 7 8 =>
            // 1 2 3 5 6 t n1 n2 n3 7 8
            auto beg = route_.begin() + start_pos;
            auto mid = route_.begin() + end_pos + 1;
            auto end = route_.begin() + target_pos + 1;
            std::rotate(beg, mid, end);

            for (auto i = start_pos; i <= target_pos; ++i) {
                positions_[route_[i]] = i;
            }
    ...
    */
    ...
    void two_opt_nn(const ProblemInstance &instance,
        ...
        cost_ += cost_change;
        //cout<<"changes "<<changes_count<<' '<<checklist.size()<<'\n';
    }

    uint32_t count_length(uint32_t start_, uint32_t end_) {
        auto pos_start = positions_.at(start_);
        auto pos_end = positions_.at(end_);
        return (pos_start <= pos_end) ? pos_end - pos_start : pos_end + positions_.size() - pos_start;
    }
...
public:
    ...
    EnhancedRoute(EnhancedRoute &b)
        :   //Route(),
            adj(b.adj){
        cost_ = b.cost_;
    }

    void initial_adj(vector <uint32_t> &route_) {
        adj[0].resize(route_.size());
        adj[1].resize(route_.size());
        for (uint32_t i = 0; i < route_.size(); i++) {
            auto u = route_[i];
            auto next_u = i < route_.size() - 1 ? route_[i + 1] : route_[0];
            adj[0][u] = next_u;
            adj[1][next_u] = u;
    ...
    void init_route(uint32_t dimension) {
        route_=get_route();
        positions_.resize(dimension);
        for (uint32_t i = 0; i < dimension; i++) {
            positions_[route_[i]] = i;
        }
    }

    void update_route(uint32_t start_node, uint32_t end_node) {
        auto pos = positions_.at(start_node);
        while (start_node != end_node) {
            auto next_node = get_next(start_node);

        }
    }

    std::vector <uint32_t> get_route() {
    ...
    void enhanced_relocate_node(const ProblemInstance &instance, uint32_t target, uint32_t node) {
        assert(node != target);
        assert(node < instance.dimension_);
        assert(target < instance.dimension_);

        if (get_next(target) == node) { return ; }

        const auto node_pred = get_prev(node);
        const auto node_succ = get_next(node);
        const auto target_succ = get_next(target);

        adj[0][target] = node;
        adj[1][node] = target;
        adj[0][node] = target_succ;
        adj[1][target_succ] = node;
        adj[0][node_pred] = node_succ;
        adj[1][node_succ] = node_pred;
        
        assert(get_succ(target) == node);

        // We are removing these edges:
        cost_ += - instance.get_distance(node_pred, node)
                 - instance.get_distance(node, node_succ)
                 - instance.get_distance(target, target_succ)
                 + instance.get_distance(node_pred, node_succ)
                 + instance.get_distance(target, node)
                 + instance.get_distance(node, target_succ);
    }

    uint32_t get_next(uint32_t node) const {
        return adj[0][node];
    }

    uint32_t get_prev(uint32_t node) const {
        return adj[1][node];
    }

    bool contains_edge(uint32_t a, uint32_t b) const {
        return b == get_next(a) || b == get_prev(a);
    }
    ...
    std::vector <uint32_t> get_next_nodes(uint32_t start_node, uint32_t max_size, uint32_t bound) {
        std::vector<uint32_t> nodes;
        while (start_node != bound && max_size > 0) {
            nodes.push_back(start_node);
            start_node = get_next(start_node);
            max_size --;
        }
        return nodes;
    }

    std::vector <uint32_t> get_prev_nodes(uint32_t start_node, uint32_t max_size, uint32_t bound) {
        std::vector<uint32_t> nodes;
        while (start_node != bound && max_size > 0) {
            nodes.push_back(start_node);
            start_node = get_prev(start_node);
            max_size --;
        }
        return nodes;
    }

    void swap_three_opt(uint32_t a, uint32_t b, uint32_t c) {
        auto next_a = get_next(a);
        auto next_b = get_next(b);
        auto next_c = get_next(c);
        adj[0][a] = next_b;
        adj[1][next_b] = a;
        adj[0][b] = next_c;
        adj[1][next_c] = b;
    ...
    void three_opt_nn(const ProblemInstance &instance,
                    std::vector<uint32_t> &checklist,
                    uint32_t nn_list_size) {
        const uint32_t MaxChanges = adj[0].size();
        uint32_t changes_count = 0;
        ...
        while (checklist_pos_pos < checklist.size() && changes_count < MaxChanges) {
            ...
            auto a_next = get_next(a);
            auto a_pred = get_prev(a);
            
            double max_diff = -1;
            std::array<uint32_t, 3> move;

            const auto &nn_list = instance.get_nearest_neighbors(a, nn_list_size);
            auto dist_a_to_next = instance.get_distance(a, a_next);
            auto dist_a_to_pred = instance.get_distance(a, a_pred);
            for (auto b : nn_list) {
                if (contains_edge(a, b)) continue;
                if (instance.get_distance(a, b) >= dist_a_to_next) break;
                auto b_pred = get_prev(b);
                
                for (auto c : get_next_nodes(b, 5, a)) {
                    auto c_next = get_next(c);
                    double diff = instance.get_distance(a, a_next)
                            + instance.get_distance(b_pred, b)
                            + instance.get_distance(c, c_next)
                            - instance.get_distance(a, b)
                            - instance.get_distance(c, a_next)
                            - instance.get_distance(b_pred, c_next);
                    if (diff > max_diff) {
                        move = { a, b_pred, c};
                        max_diff = diff;
                    }
                ...
                for (auto c : get_prev_nodes(a_pred, 5, b_pred)) {
                    auto c_next = get_next(c);
                    double diff = instance.get_distance(a, a_next)
                            + instance.get_distance(b_pred, b)
                            + instance.get_distance(c, c_next)
            ...
            /*for (auto b : nn_list) {
                ...
                for (auto c : get_pred_nodes(b, 2, a)) {
                    auto c_pred = get_pred(c);
                    double diff = instance.get_distance(a_pred, a)
                            + instance.get_distance(b, b_next)
                            + instance.get_distance(c_pred, c)
                            - instance.get_distance(a_pred, c)
                            - instance.get_distance(b, a)
                            - instance.get_distance(c_pred, b_next);
                    if (diff > max_diff) {
                        move = { a_pred, c_pred, b};
                        max_diff = diff;
                    }
                }

                for (auto c : get_next_nodes(a_next, 2, b_next)) {
                    auto c_pred = get_pred(c);
                    double diff = instance.get_distance(a_pred, a)
                            + instance.get_distance(b, b_next)
                            + instance.get_distance(c_pred, c)
            ...
            if (max_diff > 0) {
                swap_three_opt(move[0], move[1], move[2]);

                move[0] = get_prev(move[1]);
                for (auto x : move) {
                    if (std::find(checklist.begin() + static_cast<int32_t>(checklist_pos_pos),
                                checklist.end(), x) == checklist.end()) {
                        checklist.push_back(x);
        ...
        /*if (instance.calculate_route_length(get_route()) != cost_) {
            cout << "caaaaaaaaaaaaaaaaaaaaa\n";
        }*/
        //cout<<"changes "<<changes_count<<' '<<checklist.size()<<'\n';
    }
    ...
    /*void two_opt_swap(const ProblemInstance &instance, uint32_t u, uint32_t v, uint32_t next_u, uint32_t next_v) {
        auto pred_v = get_pred(v);
        adj[0][u] = v; //break u->next_u, connect u->v
        adj[1][v] = u; //connect v<-u
        adj[0][next_u] = next_v; //connect next_u->next_v
        adj[1][next_v] = next_u; //break v->next_v, connect next_u->next_v

        uint32_t dem = 0;
        while (pred_v != u) {
            auto new_pred = get_pred(pred_v);
            //reverse edge pred_v -> v
            adj[0][v] = pred_v; 
            adj[1][pred_v] = v;
            v = pred_v;
            pred_v = new_pred;
            dem ++;
            if (dem > adj[0].size()) {
                cout<<"caaaaaaaaaaaaaaaaaaaa\n";
                break;
            }
        }
        /*update_position(adj[0].size(), 0);
        if (!instance.is_route_valid(route_)) {
            cout<<"vvvvvvvvvvvvvvvvvvvvvvv\n";
        }
        if (!check_list_next() || !check_list_prev()) {
            cout<<"zzzz wrong list\n";
        }
    }*/
    ...
    void two_opt_nn(const ProblemInstance &instance,
        ...
        while (checklist_pos_pos < checklist.size() && changes_count < MaxChanges) {
            ...
            auto dist_a_to_next = instance.get_distance(a, a_next);// instance.get_distance(a, a_next);
            auto dist_a_to_prev = instance.get_distance(a_prev, a);
            ...
            for (auto b : nn_list) {
                ...
                if (dist_a_to_next > dist_ab) {
                    ...
                    if (diff > max_diff) {
                        //cout<<"hhh "<<dist_a_to_next<<' '<<dist_a_to_prev<<' '<<get_dist_to_succ(b)<<' '<<dist_ab<<'\n';
                        move = { a_next, b_next, a, b };
                        max_diff = diff;
                    }
                } else {
            ...
            if (max_diff > 0) {
                //cout<<"zzz "<<checklist.size()<<'\n';
                //cout<<"zzz "<<move[2] << ' '<<move[3]<<' '<<move[0]<<' '<<move[1]<<' '<<changes_count<<'\n';
                //cout<<"cost "<<cost_change<<' '<<max_diff<<'\n';
                //cout<<"changes "<<changes_count<<'\n';
                //two_opt_swap(instance, move[2], move[3], move[0], move[1]);
                flip_route_section(move[0], move[1]);
                //initial_adj();
                /*for (auto x : move) {
                    if (std::find(checklist.begin() + static_cast<int32_t>(checklist_pos_pos),
...
run_mfaco(const ProblemInstance &problem,
    ...
    double reset_route_time = 0;
    uint32_t total_check_list = 0;
    uint32_t total_relocated = 0;
    #pragma omp parallel default(shared)
    {
        // Endpoints of new edges (not present in source_route) are inserted
        // into ls_checklist and later used to guide local search
        ...
        for (int32_t iteration = 1 ; iteration <= iterations ; ++iteration) {
            ...
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
                ...
                while (new_edges < target_new_edges && visited_count < dimension) {
                    ...
                    total_relocated += (route.get_succ(curr) != sel);
            ...
            #pragma omp master
            {
                ...
                if (iteration % 10 == 0) {
                    cout<<"relocate time "<<relocate_time/opt.threads_<<'\n';
                    cout<<"ls time "<<ls_time/opt.threads_<<'\n';
                    cout<<"reset route time "<<reset_route_time/opt.threads_<<'\n';
                    cout<<"average check list " <<(double) total_check_list / (double) (iteration  * ants_count)<<'\n';
                    cout<<"total relocated "<< (double) total_relocated / (double) (iteration  * ants_count)<<'\n';
                }
            }
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_fast_mfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int32_t iteration = 1 ; iteration <= iterations ; ++iteration) {
            ...
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
                ...
                auto curr_node = start_node;
                uint32_t visited_count = 1;
                std::vector <uint32_t> changes[2];
                auto last_node = start_node;

                while (new_edges < target_new_edges && visited_count < dimension) {
                    auto curr = curr_node;
                    if (new_edges == 0) {
                        visited.set_bit(route.get_succ(start_node));
                        visited.set_bit(route.get_pred(start_node));
                    }
                    auto sel = select_next_node(pheromone, heuristic,
                                                problem.get_nearest_neighbors(curr, cl_size),
                                                nn_product_cache,
                    ...
                    const auto sel_pred = route.get_pred(sel);
                    if (new_edges == 0) {
                        visited.clear_bit(route.get_succ(start_node));
                        visited.clear_bit(route.get_pred(start_node));
                    }
                    // ant.visit(sel);
                    visited.set_bit(sel);
                    ++visited_count;
                    ...
                    if (!local_source.contains_edge(curr, sel)) {
                        ...
                        */
                        ...
                        if (!contains(ls_checklist, sel)) { ls_checklist.push_back(sel); total_check_list ++;}
                        if (!contains(ls_checklist, sel_pred)) { ls_checklist.push_back(sel_pred); total_check_list++;}
                        changes[0].push_back(curr);
                        changes[1].push_back(sel);
                    }
                    curr_node = sel;
                }
                total_visited += visited_count;
                ...
                if (changes[0].size() > 0) {
                    //cout<<"--------------------------------------------\n";
                    for (uint32_t i = 0; i < changes[0].size(); i++) {
                        if (i == changes[0].size() - 1) {
                            route.relocate_node(changes[0][i], changes[1][i]);
                        } else {
                            route.quick_relocate(changes[0][i], changes[1][i], changes[0][i+1]);
                        }

                    }

                    
                   /* if (!problem.is_route_valid(route.route_) || route.cost_ != problem.calculate_route_length(route.route_)) {
                        cout<<"caaaaaaaaaaaaaaaaaaaaaaaaaaaa\n";
                        exit(0);
                    }*/
            ...
            #pragma omp master
            {
                ...
                if (iteration % 100 == 0) {
                    cout<<"relocate time "<<relocate_time/opt.threads_<<' '<<iteration<<'\n';
                    cout<<"ls time "<<ls_time/opt.threads_<<'\n';
                    cout<<"reset route time "<<reset_route_time/opt.threads_<<'\n';
                    cout<<"average check list " <<(double) total_check_list / (double) (iteration  * ants_count)<<'\n';
                }
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_quick_mfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int32_t iteration = 1 ; iteration <= iterations ; ++iteration) {
            ...
            EnhancedRoute local_source;
            local_source.initial_adj(source_solution->route_);
            local_source.cost_ = source_solution->cost_;
            ...
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
                ...
                EnhancedRoute route { local_source };  // We use "external" route and only copy it back to ant
                reset_time += omp_get_wtime() - start_p;
                auto start_node = get_rng().next_uint32(dimension);
                // ant.visit(start_node);
                visited.clear();
                visited.set_bit(start_node);
                ...
                while (new_edges < target_new_edges && visited_count < dimension) {
                    ...
                    const auto sel_pred = route.get_prev(sel);
                    ...
                    double start_r = omp_get_wtime();
                    auto old_next = route.get_next(sel);
                    route.enhanced_relocate_node(problem, curr, sel);  // Place sel node just after curr node
                    ...
                    if (!local_source.contains_edge(curr, sel)) {
                        ...
                        */
                        ...
                        if (!contains(ls_checklist, sel)) { ls_checklist.push_back(sel); total_check_list ++;}
                        if (!contains(ls_checklist, sel_pred)) { ls_checklist.push_back(sel_pred); total_check_list++;}
                        if (route.count_length(start_node, old_next) > route.count_length(start_node, last_node)) 
                            last_node = old_next;
                    }
                    curr_node = sel;
                }
                total_visited += visited_count;
                /*if (problem.calculate_route_length(route.get_route()) != route.cost_) {
                    cout << "caaaaaaaaaaaaaaaaaaaaaaaaaaaa\n";
                }*/

                double start_init = omp_get_wtime();
                //route.init_route(dimension);
                //route.update_route(start_node, last_node);
                init_time += omp_get_wtime() - start_init;


                if (use_ls) {
                    double start = omp_get_wtime();
                    route.three_opt_nn(problem, ls_checklist, opt.ls_cand_list_size_);
                    ls_time += omp_get_wtime() - start;
                    //route.initial_adj(route.route_);
                ...
                // updating the current local source solution.
                if (opt.source_sol_local_update_ && route.cost_ < local_source.cost_) {
                    local_source = EnhancedRoute{route};
                    local_source.cost_ = route.cost_;
            ...
            #pragma omp master
            {
                ...
                if (iteration % 10 == 0) {
                    ...
                    cout<<"ls time "<<ls_time/opt.threads_<<'\n';
                    cout<<"average check list " <<(double) total_check_list / (double) (iteration  * ants_count)<<'\n';
                    cout<<"reset time "<<reset_time/opt.threads_<<'\n';
                     cout<<"init route time "<<init_time/opt.threads_<<'\n';
                }
            }
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_smfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
...
Route adjust_route(Route &route, uint32_t start_node) {
    double dp[2002][12];
    for (uint32_t mask = 0; mask < (1<<10); mask++)
        for (uint32_t i = 0; i < 10; i++) 
            dp[mask][i] = 1e18;
    vector <uint32_t> node_list;
    double initc = route.cost_;
    auto curr = start_node;
    for (uint32_t i = 0; i < 10; i++) {
        auto next_node = route.get_succ(curr);
        node_list.push_back(next_node); 
        initc -= route.cost_fn_(curr, next_node);
        curr = next_node;
    }
    auto end_node = route.get_succ(curr);
    initc -= route.cost_fn_(curr, end_node);
    for (uint32_t i = 0; i < 10; i++) {
        dp[1 << i][i] = initc + route.cost_fn_(start_node, node_list[i]) + route.cost_fn_(node_list[i], end_node);
    }
    for (uint32_t mask = 1; mask < (1<<10); mask++) 
        for (uint32_t i = 0; i < 10; i++) {
            if (getbit(mask, i) == 0) continue;
            if (dp[mask][i] < 1e18) continue;
            for (uint32_t j = 0; j < 10; j++) {
                if (getbit(mask, j) == 0 || j == i) continue;
                double new_cost = dp[mask ^ (1 << i)][j] - route.cost_fn_(node_list[j], end_node) + route.cost_fn_(node_list[j], node_list[i]) + route.cost_fn_(node_list[i], end_node);
    ...
    uint32_t sel = 9;
    double min_cost = dp[mask][9];
    for (uint32_t i = 0; i < 10; i++)
        if (min_cost > dp[mask][i]) {
            sel = i;
            min_cost = dp[mask][i];
        }
    for (uint32_t i = 0; i < 10; i++) {
        ...
        for (auto v: node_list)
            if(v == node_list[sel]) ok = true;
        if (!ok) cout<<"cccc\n";
        for (auto v: result_nodes)
            if(v == node_list[sel]) cout<<"gggg\n";
        result_nodes.push_back(node_list[sel]);
        for (uint32_t j = 0; j < 10; j++) {
            if (getbit(mask, j) == 0 || j == sel) continue;
            if (dp[mask][sel] == dp[mask ^ (1 << sel)][j] - route.cost_fn_(node_list[j], end_node) + route.cost_fn_(node_list[j], node_list[sel]) + route.cost_fn_(node_list[sel], end_node)) {
                mask = mask ^ (1 << sel);
                sel = j;
                break;
            }
    ...
    double init_cost = route.cost_;
    
    curr = start_node;
    for (uint32_t i = 0; i < 10; i++) {
        auto next_node = route.get_succ(curr);
        auto new_node = result_nodes[10 - 1 - i];
        route.relocate_node(curr, new_node);
        curr = new_node;
    }
    curr = start_node;
     for (uint32_t i = 0; i < 10; i++) {
        auto next_node = route.get_succ(curr);
        if (next_node != result_nodes[10 - 1 - i]) cout<<"llll\n";
        curr = next_node;
    }

    if (route.cost_ != min_cost) cout<<"mismatch\n";
    if (route.cost_ > init_cost) cout <<"Bad solution\n";
    return route;
}
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_first_new_edge_mfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
        ...
        for (int32_t iteration = 1 ; iteration <= iterations ; ++iteration) {
            ...
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
                ...
                while (new_edges < target_new_edges && visited_count < dimension) {
                    ...
                    assert(route.get_succ(curr) == sel);  // We should have (curr, sel) edge
                    if (visited_count == 2 && local_source.contains_edge(curr, sel)) {
                        cout<<"bbbb\n";
                    }
                    if (!local_source.contains_edge(curr, sel)) {
                        /*
                        ...
                        */
                        new_edges += 1;
                        //if (new_edges == 1 && visited_count != 1) cout<<"zzzzz "<<visited_count<<"\n";
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_first_new_improved_mfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
        ...
        for (int32_t iteration = 1 ; iteration <= iterations ; ++iteration) {
            ...
            for (uint32_t ant_idx = 0; ant_idx < ants.size(); ++ant_idx) {
                ...
                while (new_edges < target_new_edges && visited_count < dimension) {
                    ...
                    double q = get_rng().next_float();
                    bool zz = false;
                    bool yy = false;
                    if (q < 0.5) {
                        if (new_edges > 0 && !visited.is_set(local_source.get_succ(curr))) {
                            zz = true;
                            visited.set_bit(local_source.get_succ(curr));
                        }

                        if (new_edges > 0 && !visited.is_set(local_source.get_pred(curr))) {
                            yy = true;
                            visited.set_bit(local_source.get_pred(curr));
                        }
                    }
                    ...
                    if (!local_source.contains_edge(curr, sel)) {
                        ...
                        */
                        new_edges += 1;
                        if (new_edges == 1 && visited_count != 2) cout<<"zzzzz "<<visited_count<<"\n";
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_synthetic_smfaco(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
...
template<typename ComputationsLog_t>
std::unique_ptr<Solution> 
run_final_improved(const ProblemInstance &problem,
                const ProgramOptions &opt,
                ComputationsLog_t &comp_log) {
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        } else if (args.algorithm_ == "smfaco") {
            alg = run_smfaco;
        ...
        } else if (args.algorithm_ == "first_new_edge_mfaco") {
            alg = run_first_new_edge_mfaco;

            if (args.ants_count_ == 0) {
                auto r = 4 * sqrt(problem.dimension_);
                args.ants_count_ = static_cast<uint32_t>(lround(r / 64) * 64);
            }
        } else if (args.algorithm_ == "synthetic_smfaco") {
            alg = run_synthetic_smfaco;

            if (args.ants_count_ == 0) {
                auto r = 4 * sqrt(problem.dimension_);
                args.ants_count_ = static_cast<uint32_t>(lround(r / 64) * 64);
            }
        } else if (args.algorithm_ == "final_improved") {
            alg = run_final_improved;
        ...
        } else if (args.algorithm_ == "quick_mfaco") {
            alg = run_quick_mfaco;
        ...
        } else if (args.algorithm_ == "fast_mfaco") {
            alg = run_fast_mfaco;
