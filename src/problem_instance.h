...
struct ProblemInstance {
    ...
    void compute_nn_lists(uint32_t nn_count) {
        ...
        all_nearest_neighbors_.resize(dimension_ * total_nn_per_node_);

        if (kdtree_ != nullptr) {
            auto &shared_kdtree = *kdtree_;

            #pragma omp parallel default(none) shared(shared_kdtree, total_nn_per_node_, all_nearest_neighbors_)
            {
                KDTree kdtree = shared_kdtree;  // private copy
                uint32_t n = kdtree.get_points_count();

                #pragma omp for schedule(static)
                for (uint32_t node = 0; node < n; ++node) {
                    auto offset = node * total_nn_per_node_;
                    for (uint32_t j = 0; j < total_nn_per_node_; ++j) {
                        auto pt_idx = kdtree.nn_bottom_up(node);
                        all_nearest_neighbors_[offset + j] = pt_idx;
            ...
            for (uint32_t node = 0; node < dimension_; ++node) {
                ...
                const auto offset = node * total_nn_per_node_;
                for (uint32_t i = 0; i < total_nn_per_node_; ++i) {
                    all_nearest_neighbors_[offset + i] = neighbors.at(i);
                }
            }
        }
    }
    ...
    std::function<double (uint32_t, uint32_t)> get_distance_fn() const {
        return [this](uint32_t a, uint32_t b) {
            return this->get_distance(a, b);
        };
    }
